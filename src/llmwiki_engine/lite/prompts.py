            ...
            **({"granularity_stats": granularity_stats.model_dump(mode="json")} if granularity_stats is not None else {}),
            ...
            "instructions": [
                *CHINESE_OUTPUT_RULES,
                ...
                "先从 raw_text 抽取原子 claims；claim 是可以独立验证的最小事实单元，不是页面。",
                "原子 claim 粒度判断标准：去掉任何一部分后剩余信息不再成立，说明粒度太细，应合并；包含两个或以上可独立验证的事实，说明粒度太粗，应拆分；每个 claim 只回答一个是什么、多少、怎么做或为什么的问题。",
                "先逐条提取原子 claim，不要预设结构或预判页面。提取完成后再将 claims 聚类为 content_units，聚类时不要回头修改已提取的 claims。",
        ...
        payload={
            ...
            "instructions": [
                *CHINESE_OUTPUT_RULES,
                ...
                "必须填写 okf_type 字段。okf_type 描述这个知识概念本身所属的领域种类（如 Protocol、Product、Person、Team、Framework、Tool、Event、Metric、Reference、Methodology 等），回答“这是什么”；注意它与 page_type 语义不同：page_type 是页面在知识库中的结构角色（concept/overview/comparison/design/entity），回答“这个页面怎么组织”。虽然 frontmatter 由引擎生成，但 okf_type 的值需要你根据页面内容判断后提供；如果该概念没有更具体的领域种类，okf_type 填 page_type 的值即可。",
                ...
                "正文可以写 0-{limit} 条 Obsidian wikilink，但只在阅读语境确实需要跳转理解时使用，不要为了凑数而链接。".format(limit=page_plugin.body_wikilink_limit) if page_plugin.body_wikilink_limit > 0 else "正文不要写 Obsidian wikilink。",
    ...
    payload.update(
        {
            ...
            "instructions": [
                *instructions,
                ...
                f"正文 Obsidian wikilink 最多 {page_plugin.body_wikilink_limit} 条，且必须来自 allowed_body_link_targets.path；也可以不写正文链接。",
        ...
        payload={
            ...
            "instructions": [
                *CHINESE_OUTPUT_RULES,
                ...
                "不要写 Related 或 相关页面章节；引擎会统一生成。",
                f"正文 Obsidian wikilink 最多 {page_plugin.body_wikilink_limit} 条；也可以不写正文链接。",
                "正文中不得包含 raw、sources、logs、index、Source_* 或 source page 的 wikilink、Markdown link、HTML href。",
                "不要因为 source_refs 或 frontmatter 包含 raw 路径就判 covered；必须看正文内容。",
            ],
        },
