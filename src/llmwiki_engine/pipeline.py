    ...
    *,
    ...
    for page in artifact.pages:
        ...
        if not page_claims:
            rewritten_pages.append(page)
            continue
        updated_body = page.body_markdown
        changed = False
        for claim in page_claims:
            target_section_key = "detail"
            if not detail_contains_example_heading(updated_body):
                continue
        ...
        if changed:
            rewritten_pages.append(page.model_copy(update={"body_markdown": updated_body}))
        else:
            rewritten_pages.append(page)
    report = {
...
def detail_contains_example_heading(body: str) -> bool:
    for line in body.splitlines():
        match = re.match(r"^\s{0,3}#{1,6}\s+(.+?)\s*$", line)
        if match and draft_body_heading_section_key(match.group(1)) == "examples":
            return True
    return False
...
def position_inside_fenced_code_block(text: str, position: int) -> bool:
    ...
    for line in text.splitlines(keepends=True):
        ...
        if fence_char:
            if _draft_validation.closing_fence_line(stripped_newline, fence_char, fence_length):
                fence_char = ""
                fence_length = 0
        elif match := _draft_validation.opening_fence_line(stripped_newline):
            marker = match.group("marker")
            fence_char = marker[0]
            fence_length = len(marker)
        cursor = line_end
