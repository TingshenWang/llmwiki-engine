...
def test_openai_compatible_payload_uses_json_schema_and_sanitizes_key(tmp_path: Path, monkeypatch) -> None:
...
def test_mock_fixture_provider_writes_prompt_and_provider_result_artifacts(tmp_path: Path) -> None:
    ...
    manifest = run_ingest(vault, raw, slug="mock-provider", emit_progress=False)
